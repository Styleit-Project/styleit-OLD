...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::STYL)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("STYL")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeStyleitAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeStyleitAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeStyleitAmount = strAnonymizeStyleitAmount.remove(strAnonymizeStyleitAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeStyleitAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeStyleitAmount * COIN) nMaxToAnonymize = nAnonymizeStyleitAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeStyleitAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeStyleitAmount));
        strAnonymizeStyleitAmount = strAnonymizeStyleitAmount.remove(strAnonymizeStyleitAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeStyleitAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
